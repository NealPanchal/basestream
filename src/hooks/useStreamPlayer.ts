...
const PROVIDER_CACHE_KEY = 'basestream_provider_cache';
...
export function useStreamPlayer(
  ...
  const [roster, setRoster] = useState<ProviderId[]>(() => {
    if (typeof window !== 'undefined') {
      ...
      if (cached && adaptive.includes(cached as ProviderId)) {
        return [cached as ProviderId, ...adaptive.filter(p => p !== cached)];
      }
      return adaptive;
    }
  ...
  const currentProvider = roster[providerIndex] ?? 'vidsrc';
  ...
  const handleIframeLoad = useCallback(() => {
    ...
    // Lock provider and cache on successful load
    setProviderLocked(true);
    setCachedProvider(tmdbId, type, season, episode, currentProvider);

    // Record success in adaptive ranking
    rankRecordSuccess(currentProvider, durationMs);
    setProviderScores(getProviderScores());
  }, [currentProvider, embedUrl, loadStartMs, clearIframeTimeout, tmdbId, type, season, episode]);
